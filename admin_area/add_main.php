...
$rune_teacher = mysqli_query($con,$sel_teacher); 
...
echo"<title>Add Main-$college</title>";
...
<body>
	...
	<section id='secondsection'>
		<div id="form">
			<h1 style='text-align:center;'>Add Department</h1><br><br>
			<form align='center' method="post">
				<select class="form-control" name="dept_id" required>
											<option value="">Select Department ID</option>
														<?php
													$get_cats="select * from departments order by dept_id asc";
													$run_cats=mysqli_query($con,$get_cats);
													while($row_cats=mysqli_fetch_array($run_cats))
													{
														$my_name=$row_cats['dept_id'];
				...
				<select class="form-control" name="dept_name" required>
											<option value="">Select Department</option>
														<?php
													$get_cats="select * from departments order by dept_name asc";
													$run_cats=mysqli_query($con,$get_cats);
													while($row_cats=mysqli_fetch_array($run_cats))
													{
													...
													?></select><br>
				<input class="form-control" type="text" name="person_name" placeholder="Enter HOD/Head Person Name" required="required" /><br><br>
				<input class="form-control" type="text" name="user" placeholder="Enter Username" required="required" /><br><br>
				<input class="form-control" type="text" name="pass" placeholder="Enter Password" required="required" /><br><br>
				<button  type="submit" class="btn btn-primary" name="add_dept">Add</button>
			</form>
		</div>
	</section>
</body>
...
if(isset($_POST['add_dept'])){
	...
	$user=$_POST['user'];
	$pass=$_POST['pass'];
	$add_dept = "insert into main_people (dept_id,dept_name,panel_name,panel_username,panel_password) values ('$dept_id','$dept_name','$person_name','$user','$pass')";
	$run_dept = mysqli_query($con, $add_dept); 
	if($run_dept){
		echo "<script>alert('Person Added!!!')</script>";
		echo "<script>window.open('admin_panel?name=$college','_self')</script>";
	}
		else{
			echo "<script>alert('College Name or Password is incorrect, try again')</script>";
			}
	}
}
else{
