...
<body>
	...
	<section id='secondsection'>
		<div id="form">
			<h1 align='center'>Delete Faculty</h1>
			<h4 align='center'>Select name and teacher id of the respective teacher you want to delete.</h4>
			<br><br>
			<form align='center' method="post">
				<select  class="form-control" type="number" name="teach_name" required>
					<option value="">Select Teacher</option>
					<?php
							$get_cats="select distinct name from teachers order by name asc";
...
if(isset($_POST['add_sem'])){
	...
	$sel_id=mysqli_query($con,"select name as pop from teachers where teacher_id='$t_id'");
	$chec_name=mysqli_fetch_array($sel_id);
	$check_name=$chec_name['pop'];
	if($check_name==$teach_name){
	$itis=$teach_name.$t_id;
	$itis = str_replace(' ', '',$itis);
$itis = str_replace('.', '',$itis);
$itis = str_replace('-', '',$itis);
$itis = preg_replace('/[^A-Za-z0-9\-]/', '',$itis);
$itis=strtolower($itis);
$delete_data="delete from teachers where teacher_id='$t_id' and name='$teach_name'";
				...
				if($result !== false)
			{
				  ...
				  if($result->num_rows > 0)
				{
					...
					while($row = $result->fetch_assoc()) {
						...
						$del_each="delete from $now_tab where teacher_name='$teach_name'";
						$run_each=mysqli_query($con,$del_each);
					  if( $itis==$now_tab)
					  {
						  $del_tab="drop table $itis";
						$run_del=mysqli_query($con,$del_tab); 
						} 
					}
...
$p=mysqli_affected_rows($run_delte);
if($run_delte){
	...
	echo"<script>
					...
					p1.textContent='Incorrect Name';
					p2.textContent='Incorrect Teacher Id';
				</script>";
	}
	}
}
